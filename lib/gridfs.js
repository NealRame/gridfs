...
GridFs.prototype.open = function(path, flags, mode, callback) {
    ...
    var fd = new this.mongo.GridStore(this.db, id, id.toString(), flags, {
        root: this.root,
    });
...
GridFs.prototype.read = function(fd, buffer, offset, length, position, callback) {
    ...
    if (_.isNumber(position)) {
        ...
        fd.read(length, function(err, data) {
            if (! err) {
                data.copy(buffer, offset);
            }
            callback(err, data.length, buffer);
        });
    }
};
...
GridFs.prototype.writeFile = function(filename, data, options, callback) {
    ...
    return this.openAsync(filename, 'w')
        .then(function(fd) {
    ...
    .map(function(method_name) {
        return [
            ...
            function() {
                ...
                var method = GridFs.prototype[method_name];
                return new Promise(function(resolve, reject) {
                    method.apply(self, args.concat(function(err) {
                        if (err) {
                            reject(err);
                        } else {
                            resolve.apply(null, _.rest(arguments));
                        }
                    }));
                });
            }
